...
int32_t rubraview_bytes_per_pixel(rubraview_pixel_format_t fmt) {
    switch (fmt) {
        case RUBRAVIEW_PIXFMT_RGBA8:
        case RUBRAVIEW_PIXFMT_BGRA8:
            return 4;
        case RUBRAVIEW_PIXFMT_GRAY8:
            return 1;
        case RUBRAVIEW_PIXFMT_RGBA16F:
            return 8;
        default:
            return 4;
    }
...
uint8_t *rubraview_pixbuf_at(const rubraview_pixbuf_t *pb, int32_t x, int32_t y) {
    if (!rubraview_pixbuf_is_valid(pb)) return NULL;
    if (x < 0 || x >= pb->width || y < 0 || y >= pb->height) return NULL;
    return pb->pixels + ((ptrdiff_t)y * pb->stride) + ((ptrdiff_t)x * rubraview_bytes_per_pixel(pb->format));
}

void rubraview_pixbuf_clear(rubraview_pixbuf_t *pb, uint32_t color_hex) {
    if (!rubraview_pixbuf_is_valid(pb)) return;

    int32_t bpp = rubraview_bytes_per_pixel(pb->format);
    if (bpp == 4) {
...
rubraview_pixbuf_t rubraview_pixbuf_clone(proven_arena_t *arena, const rubraview_pixbuf_t *src) {
    if (!rubraview_pixbuf_is_valid(src) || arena == NULL) {
        return (rubraview_pixbuf_t){0};
    }
...
rubraview_pixbuf_t rubraview_pixbuf_subview(const rubraview_pixbuf_t *src, int32_t x, int32_t y, int32_t w, int32_t h) {
    if (!rubraview_pixbuf_is_valid(src)) return (rubraview_pixbuf_t){0};
    if (x < 0 || y < 0 || w <= 0 || h <= 0) return (rubraview_pixbuf_t){0};
    if (x >= src->width || y >= src->height) return (rubraview_pixbuf_t){0};
    ...
    if (w <= 0 || h <= 0) return (rubraview_pixbuf_t){0};
...
rubraview_pixbuf_t rubraview_pixbuf_convert(proven_arena_t *arena, const rubraview_pixbuf_t *src, rubraview_pixel_format_t dst_format) {
    ...
    rubraview_pixbuf_t dst = rubraview_pixbuf_create(arena, src->width, src->height, dst_format);
    if (!rubraview_pixbuf_is_valid(&dst)) return (rubraview_pixbuf_t){0};
    ...
    for (int32_t y = 0; y < src->height; ++y) {
        ...
        if (src->format == RUBRAVIEW_PIXFMT_RGBA8 && dst_format == RUBRAVIEW_PIXFMT_BGRA8) {
        ...
        } else if (src->format == RUBRAVIEW_PIXFMT_BGRA8 && dst_format == RUBRAVIEW_PIXFMT_RGBA8) {
            for (int32_t x = 0; x < src->width; ++x) {
                d[x * 4 + 0] = s[x * 4 + 2]; /* R */
                d[x * 4 + 1] = s[x * 4 + 1]; /* G */
                d[x * 4 + 2] = s[x * 4 + 0]; /* B */
        ...
        } else if ((src->format == RUBRAVIEW_PIXFMT_RGBA8 || src->format == RUBRAVIEW_PIXFMT_BGRA8) && dst_format == RUBRAVIEW_PIXFMT_GRAY8) {
            bool is_bgra = (src->format == RUBRAVIEW_PIXFMT_BGRA8);
            for (int32_t x = 0; x < src->width; ++x) {
        ...
        } else if (src->format == RUBRAVIEW_PIXFMT_GRAY8 && (dst_format == RUBRAVIEW_PIXFMT_RGBA8 || dst_format == RUBRAVIEW_PIXFMT_BGRA8)) {
            for (int32_t x = 0; x < src->width; ++x) {
                uint8_t g = s[x];
                d[x * 4 + 0] = g;
                d[x * 4 + 1] = g;
