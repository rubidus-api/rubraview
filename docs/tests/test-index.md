...
| T028 | R129, R130, R131 | Verify tile grid geometry, anchor clamping, detach/dock, pin, menu hierarchy and breadcrumb; a box noticed opening once (the menu then starts at its root); tile captions for submenus and switches, the current choice marked, and actions that cannot act on the page dimmed (`ui_actions.c`); `Order: L>R`; a destructive item fires on the second tap within 5 s (D-18) | `make test` | tests/test_ui_box.c | active |
...
| T030 | R133, R138 | Verify virtual scrolling, filmstrip budget interaction, picker breadcrumbs/type-ahead/multi-select; the picker keeps folders and openable files and counts the rest (D-18) | `make test` | tests/test_ui_browse.c | active |
...
| T053 | R143 | Verify the FFT against a known tone, logarithmic band placement, peak-hold decay, equaliser gains measured through the filter, ReplayGain's clipping guard, and night-mode compression; the 5 ms fade-in after a seek; sound used up on the wall clock with no device (T058) | `make test` | tests/test_audio_dsp.c | active |
...
| T069 | R148 | Keys changed in the settings window: a key added and used after a restart, a taken key refused with its holder named, keymap.ini written beside settings.ini and readable by TOML and INI | `make package`, run on Windows, F10, Keys page | docs/tests/cases/T069-keys-rebinding.md | manual |
| T070 | R148 | Playback keys (Space, arrows seek 5 s and change volume, Shift+M), Ctrl/Alt+arrows size and move the window, volume kept in settings.ini | `make package`, run on Windows with a video | docs/tests/cases/T070-playback-and-window-keys.md | manual |
| T071 | R148 | Anchors read M and T; Alt + wheel over a box sets its opacity in 5 % steps, never below 30 %; a lone Alt does not eat the next key | `make package`, run on Windows | docs/tests/cases/T071-box-anchors-opacity.md | manual |
| T072 | R148 | The toolbox's tiles follow the page (picture, video, music, CBZ), open inside the window; the menu has File (with Recent), View, Playback, Show, Help | `make package`, run on Windows | docs/tests/cases/T072-toolbox-profiles-and-menu.md | manual |
