...
| T001 | R107 | Verify `rv_pixbuf_t` allocation, bounds, clone, crop, and convert | `make test` | tests/test_pixbuf.c | active |
| T002 | R104, R121 | Verify sRGB <-> Linear RGB LUT, curves, levels, and color adjust | `make test` | tests/test_color.c | active |
| T003 | R104, R124 | Verify image resampling kernels (Bilinear, Bicubic, Lanczos) | `make test` | tests/test_resample.c | planned |
| T004 | R104, R123 | Verify spatial convolution (Gaussian blur, unsharp mask) | `make test` | tests/test_filters.c | planned |
