 ...
 * O(1) index relative to archive size) and, for STORED entries, returns
 * a zero-copy view directly into the caller-owned buffer — "decompressing"
 * a stored entry is the identity function, so there is nothing to copy.
 * `deflate` support (RV-051, D-2) is not yet vendored; a deflate entry is
 * indexed but rejected at read time with RUBRAVIEW_ZIP_ERR_UNSUPPORTED_COMPRESSION.
 * CB7/CBR are RV-052/RV-053 (M4), not this module.
 *
...
typedef enum rubraview_zip_err {
    ...
    RUBRAVIEW_ZIP_ERR_UNSUPPORTED_COMPRESSION, /* entry method is not STORED (0) */
